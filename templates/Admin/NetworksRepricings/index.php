<div class="mainwrapper">
    <div class="leftpanel">
        <?php echo $this->element('admin/sidebar'); ?>
    </div><!-- leftpanel -->

    <div class="mainpanel">
        <div class="pageheader">
            <div class="media">
                <div class="pageicon pull-left">
                    <i class="fa fa-network-wired"></i>
                </div>
                <div class="media-body">
                    <ul class="breadcrumb">
                        <li><?= $this->Html->link('<i class="glyphicon glyphicon-home"></i> Dashboard', ['controller' => 'Admins', 'action' => 'dashboard'], ['escape' => false]); ?></li>
                        <li><?= $this->Html->link('Settings', ['controller' => 'Settings', 'action' => 'index']); ?></li>
                        <li><?= $this->Html->link('Networks Repricing', ['controller' => 'NetworksRepricings', 'action' => 'index']); ?></li>
                        <li>Manage Networks Repricing</li>
                    </ul>
                    <h4>Manage Networks Repricing</h4>
                </div>
                <?php echo $this->Flash->render(); ?>
            </div><!-- media -->
        </div><!-- pageheader -->

        <div class="contentpanel">
            <div class="row">
                <div class="col-md-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            <h4 class="panel-title">Networks Repricing List</h4>
                            <p>Manage all networks repricing in the system</p>

                            <div class="search-body" style="width: 39%;">
                                <?= $this->Form->create(null, ['type' => 'get', 'valueSources' => ['query']]) ?>
                                <?= $this->Form->control('keyword', [
                                    'class' => 'form-control width200',
                                    'placeholder' => 'Enter Keyword to Search',
                                    'style' => 'float:left',
                                    'label' => false,
                                    'autocomplete' => 'off'
                                ]) ?>
                                <?= $this->Form->button('Search', ['class' => 'btn btn-primary mr5 ml10']) ?>
                                <?= $this->Form->end() ?>
                            </div>

                            <div class="panel-btn">
                                <?= $this->Html->link('<i class="fa fa-plus"></i> Add Network Repricing', ['action' => 'add'], ['class' => 'btn btn-primary', 'escape' => false]) ?>
                            </div>
                        </div><!-- panel-heading -->
                        <div class="panel-body">
                            <?php if ($networks->count() > 0): ?>
                                <div class="table-responsive">
                                    <table class="table table-striped table-bordered">
                                        <thead>
                                            <tr>
                                                <th>ID</th>
                                                <th>Name</th>
                                                <th>Status</th>
                                                <th>Programs</th>
                                                <th>Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            <?php foreach ($networks as $network): ?>
                                                <tr>
                                                    <td><?= $network->id ?></td>
                                                    <td><?= h($network->name) ?></td>
                                                    <td>
                                                        <?php if ($network->status == 1): ?>
                                                            <span class="badge badge-success">Active</span>
                                                        <?php else: ?>
                                                            <span class="badge badge-danger">Inactive</span>
                                                        <?php endif; ?>
                                                    </td>
                                                    <td>
                                                        <?php
                                                        if (!empty($network->program_id)) {
                                                            // Handle both string and array types
                                                            if (is_array($network->program_id)) {
                                                                $programIds = $network->program_id;
                                                            } else {
                                                                $programIds = explode(',', $network->program_id);
                                                            }
                                                            $programsTable = \Cake\ORM\TableRegistry::getTableLocator()->get('Programs');
                                                            $programNames = [];
                                                            foreach ($programIds as $programId) {
                                                                if (!empty($programId)) {
                                                                    $program = $programsTable->find()->where(['id' => $programId])->first();
                                                                    if ($program) {
                                                                        $programNames[] = h($program->name);
                                                                    }
                                                                }
                                                            }
                                                            echo !empty($programNames) ? implode(', ', $programNames) : 'No Programs';
                                                        } else {
                                                            echo 'No Programs';
                                                        }
                                                        ?>
                                                    </td>
                                                    <td>
                                                        <?= $this->Html->link('<i class="fa fa-edit"></i>', ['action' => 'edit', $network->id], ['class' => 'btn btn-sm btn-primary mr5', 'escape' => false]) ?>
                                                        <?= $this->Form->postLink('<i class="fa fa-trash"></i>', ['action' => 'delete', $network->id], [
                                                            'confirm' => __('Are you sure you want to delete this network repricing?'),
                                                            'class' => 'btn btn-sm btn-danger',
                                                            'escape' => false
                                                        ]) ?>
                                                    </td>
                                                </tr>
                                            <?php endforeach; ?>
                                        </tbody>
                                    </table>
                                </div>
                            <?php else: ?>
                                <div class="alert alert-warning">
                                    <strong>No Networks Repricing Found!</strong>
                                    <p>There are currently no networks repricing configured.
                                    <?= $this->Html->link('Add Network Repricing', ['action' => 'add'], ['class' => 'btn btn-primary']); ?>
                                    </p>
                                </div>
                            <?php endif; ?>
                        </div><!-- panel-body -->

                        <?php if ($this->Paginator->hasPage()): ?>
                        <div class="panel-footer">
                            <div class="row">
                                <div class="col-sm-6">
                                    <p class="text-muted">
                                        Showing <?= $this->Paginator->counter() ?>
                                    </p>
                                </div>
                                <div class="col-sm-6 text-right">
                                    <ul class="pagination pagination-sm m-0">
                                        <?= $this->Paginator->first('<< First') ?>
                                        <?= $this->Paginator->prev('< Previous') ?>
                                        <?= $this->Paginator->numbers() ?>
                                        <?= $this->Paginator->next('Next >') ?>
                                        <?= $this->Paginator->last('Last >>') ?>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <?php endif; ?>
                    </div><!-- panel -->
                </div>
            </div><!-- row -->
        </div><!-- contentpanel -->
    </div><!-- mainpanel -->
</div><!-- mainwrapper -->
