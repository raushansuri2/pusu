...
class ApisController extends AppController
{
	...
	// 	$this->getEventManager()->off($this->getMiddleware('csrf'));
	// 	die('helllo');
	// }
	...
	private function registration(){
		...
		if($this->request->data('email') !='' && $this->request->data('firstName') !='' && $this->request->data('password') !=''){
			$this->loadModel('Users');
			...
			if(!empty($usr)){
				...
				$response['msg'] = 'Email already registred with Ritevet';
					...
					if ($this->Users->save($users)) {
							...
							$message="Dear ".ucfirst($this->request->data('firstName'));
	...
	function socialLoginAction(){
		...
		if($this->request->data('socialId') !='' && $this->request->data('socialType') !=''){
			...
			if($this->request->data['image']){
				$this->request->data['profile_picture']  = $this->request->data('image');
			}
		...
		if($this->request->data('userId') !=''){
			...
			if(!empty($users)){
				...
				if(@$_FILES['profile_ID_image']['error'] == 0 && @$_FILES['profile_ID_image']['name'] !=''){
					...
					$this->request->data['profile_ID_image']=$fileName3;
					$OLDImage = $users->profileId;
				}else{
					unset($this->request->data['profile_ID_image']);
				}
//echo "<pre>";print_r($this->request->data);print_r($users); die;
				$users = $this->Users->patchEntity($users, $this->request->data, ['validate' => false]);
				$fileName = '';
				if(@$_FILES['image']['name']){
					$fileName=time().stripslashes($_FILES['image']['name']);
	...
	private function petparentregistration(){
		...
		if($this->request->data['userId'] !='' && $this->request->data['UTYPE'] !=''){
			$sendEmail = 0;
			$users = $this->Usersinformations->find()->where(['Usersinformations.userId' => $this->request->data['userId'],'Usersinformations.UTYPE' => $this->request->data['UTYPE']])->first();
			if(empty($users)){
				$sendEmail = 1;
				...
				if(@$_FILES['ownPicture']['name']){
					...
					$this->request->data['ownPicture']=$fileName1;
					$url1 = $this->profilePath.$users->ownPicture;
					@unlink($url1);
				}else{
					unset($this->request->data['ownPicture']);
				}
				...
				if(@$_FILES['BImage']['name']){
					...
					$rplc =['','',"","","","","","","","","","","","",""];
					$fileName2 = str_replace($str,$rplc,$fileName2);
					$this->request->data['BImage']=$fileName2;
					$url2 = $this->profilePath.$users->BImage;
					@unlink($url2);
				}else{
					unset($this->request->data['BImage']);
				}
				...
				if(@$_FILES['estimatePrice']['name']){
					...
					$rplc =['','',"","","","","","","","","","","","",""];
					$fileName3 = str_replace($str,$rplc,$fileName3);
					$this->request->data['estimatePrice']=$fileName3;
					$url3 = $this->profilePath.$users->estimatePrice;
					@unlink($url3);
				}else{
					unset($this->request->data['estimatePrice']);
				}
				...
				if(@$_FILES['uploadTranscript']['name']){
					...
					$this->request->data['uploadTranscript']=$fileName4;
					$url4 = $this->profilePath.$users->uploadTranscript;
					@unlink($url4);
				}else{
					unset($this->request->data['uploadTranscript']);
				}
				...
				if(@$_FILES['uploadLicense']['name']){
					...
					$this->request->data['uploadLicense']=$fileName5;
					$url5 = $this->profilePath.$users->uploadLicense;
					@unlink($url5);
				}else{
					unset($this->request->data['uploadLicense']);
				}
				...
				if(@$_FILES['uploadDocument']['name']){
					...
					$this->request->data['uploadDocument']=$fileName6;
					$url6 = $this->profilePath.$users->uploadDocument;
					@unlink($url6);
				}else{
					unset($this->request->data['uploadDocument']);
				}
				
				//pr($this->request->data); die;
				$users = $this->Usersinformations->patchEntity($users, $this->request->data, ['validate' => false]);
				$users['modified'] = date('Y-m-d H:i:s');
				if(@$this->request->data['UTYPE'] == 2 || @$this->request->data['UTYPE'] == 3 || $users->UTYPE == 2 || $users->UTYPE == 3){
					$users['verifyAdmin'] = 0;
				}
				if ($this->Usersinformations->save($users)) {

					if(@$this->request->data['service_rate']){
						$rate_data = json_decode(@$this->request->data['service_rate'], true);
						if(@$rate_data){
							$this->Typeofservicesrate->deleteAll(['Typeofservicesrate.userId'=>$this->request->data['userId']]);
							foreach($rate_data as $rate){
								$DATA_Rate['userId'] 			= $this->request->data['userId'];
					...
					if(@$this->request->data['home_service']){
						$H_S_data = json_decode(@$this->request->data['home_service'], true);
						if(@$H_S_data){
							$this->Homeservicesinfo->deleteAll(['Homeservicesinfo.userId'=>$this->request->data['userId']]);
							foreach($H_S_data as $rate){
								$DATA_HS['userId'] 				= $this->request->data['userId'];
					...
					if(@$this->request->data['Multilicenses']){
						@$this->Multilicenses->deleteAll(['Multilicenses.UTYPE'=>$users->UTYPE,'Multilicenses.userId'=>$this->request->data['userId'],'Multilicenses.userinformationId'=>$users->id]);
						$arr = json_decode($this->request->data['Multilicenses']);
						foreach($arr as $val){
								$DATA2['UTYPE'] 			= $users->UTYPE;
								$DATA2['userId'] 			= $this->request->data['userId'];
					...
					if($sendEmail ==1){
						$usersDD = $this->Users->find()->where(['Users.id'=>$this->request->data['userId']])->first();
						$to=$usersDD->email;
						//$to='raushan.kumar@evirtualservices.com';
						$subject="Welcome to RiteVet!";
						$message = "Dear " . ucfirst($usersDD->firstName) ." ". ucfirst($usersDD->lastName).",<br>";
						if($this->request->data['UTYPE'] == 2){
                        ...
                        Best Regards,<br>
                        The RiteVet Team<br>";
						}else if($this->request->data['UTYPE'] == 1){
					...
					$res = $this->returnprofile($this->request->data['userId'],$this->request->data['UTYPE']);
					$response['data'] = $res['data'];
				}
	...
	function returnprofile($userId=NULL, $UTYPE=NULL){
		...
		$American_board = Configure::read('American');
			...
			$UTYPEID = ($UTYPE) ? $UTYPE : $this->request->data('UTYPE');
	...
	function requestservice($userId=NULL){
			...
			$typeofbusiness = $this->request->data('typeofbusiness');
			if($typeofbusiness != ''){
			...
			$pet_type = @$this->request->data('pet_type');
			...
			$dog_type = @$this->request->data('dog_type');
			if($dog_type != ''){
			...
			$avl_date = @$this->request->data('avl_date');
			...
			$condition[] = ['Usersinformations.verifyAdmin' =>1,'Usersinformations.UTYPE' => $this->request->data['UTYPE']];
			if($keyword != ''){
				$condition[] = [
					'OR' => [
		...
		foreach($userservices as $val){
			...
			$Darray[$i]['video_availability'] = $this->Videochatavailability->find()->where(['Videochatavailability.userId'=>$val->userId,'Videochatavailability.UTYPE'=>$this->request->data['UTYPE']])->toArray();
	...
	function requestservicedetails(){
		...
		$this->loadModel('Typeofservicesrate');
		$American_board = Configure::read('American');
		@$USERID = ($userId) ? $userId : @$this->request->data['service_provider_id'];
		
		if(@$USERID){
			$condition[] = ['Usersinformations.userId'=> $USERID];
		}
		$condition[] = ['Usersinformations.UTYPE' => $this->request->data['UTYPE']];
	...
	function availaibilty (){
		$this->loadModel('Videochatavailability');
		if($this->request->data['vendorId'] !='' && $this->request->data['get_date'] !=''){
			$datee = date('D', strtotime($this->request->data['get_date']));
			$uper = strtoupper($datee);
			$videoavailabe = $this->Videochatavailability->find()->where(['Videochatavailability.userId'=>$this->request->data['vendorId'],'Videochatavailability.'.$uper=>1])->first();
			if(@$videoavailabe->id !=''){
				$slot = array();
				$i = 0;
	...
	function bookingrequest(){
		...
		if($this->request->data['serviceProvider'] !='' && $this->request->data['sender'] !='' && $this->request->data['service_id'] !=''){
			//pr($this->request->data); die;
			$check_D = $this->Requests->find()->where(['Requests.sender'=>$this->request->data['sender'],'Requests.time_slot'=>$this->request->data['time_slot'],'Requests.requested_service_date'=>date('Y-m-d',strtotime($this->request->data['requested_service_date']))])->first();
			if(@$check_D->id == '' || $this->request->data['frequency'] == 'Multi-Day'){
				$Booking = $this->Requests->newEntity();
				$Booking = $this->Requests->patchEntity($Booking, $this->request->data, ['validate' => false]);
				if($this->request->data['frequency'] == 'Multi-Day'){
					$Booking['requested_service_date'] = ''; 	
				}else{
					$Booking['requested_service_date'] = date('Y-m-d',strtotime($this->request->data['requested_service_date']));
				}
				$Booking['created'] 	= date('Y-m-d H:i:s');
				$Booking['modified'] 	= date('Y-m-d H:i:s');
				$Booking['sender'] 	= $this->request->data['sender'];
				$this->Requests->save($Booking);

				$vendor = $this->Users->find()->where(['Users.id'=>$this->request->data['serviceProvider']])->first();
				$userS = $this->Users->find()->where(['Users.id'=>$this->request->data['sender']])->first();
				...
				$message .= '<br><strong>Booking Date</strong>: '.@$this->request->data['requested_service_date'];
	...
	function bookingrequestlist($user_id=NULL, $status=NULL){
		...
		@$reviewTo 	= (@$this->request->data['reviewTo']) ? @$this->request->data['reviewTo'] : '';
		$pageNo 		= ($this->request->data('pageNo')) ? $this->request->data('pageNo') : '1';
		if($userId){
			if(@$this->request->data['user_type'] == 'Doctor'){
	...
	function requestaccept(){
		$this->loadModel('Requests');$this->loadModel('Users');
		if($this->request->data['requestId'] !='' && $this->request->data['status'] !='' ){
			$articles = TableRegistry::get('Requests');
			$query = $articles->query();
			$query->update()
				->set(['status' => $this->request->data['status']])
				->where(['id' => $this->request->data['requestId']])
				->execute();
			
				$bookings = $this->Requests->find()->where(['Requests.id'=>$this->request->data['requestId']])->first();
	...
	function requestdenied(){
		$this->loadModel('Requests');$this->loadModel('Users');
		if($this->request->data['requestId'] !='' && $this->request->data['status'] !='' && $this->request->data['cancelled_by'] !=''){
			$articles = TableRegistry::get('Requests');
			$query = $articles->query();
			$query->update()
				->set(['cancelled_by'=>$this->request->data['cancelled_by'], 'status' => $this->request->data['status']])
				->where(['id' => $this->request->data['requestId']])
				->execute();
	...
	function stripepaymentupdate(){
		...
		$this->loadModel('Requests'); $this->loadModel('Users');
		if($this->request->data['requestId'] !='' && $this->request->data['transactionId'] !='' && $this->request->data['userId'] !='' && $this->request->data['total_amount_with_admin_fee'] !='' && $this->request->data['providerId'] !=''){
			$articles = TableRegistry::get('Requests');
			$query = $articles->query();
			$query->update()
				->set(['status'=>4])
				->where(['id' => $this->request->data['requestId']])
				->execute();
			$request_detail = $this->Requests->find()->where(['Requests.id'=>$this->request->data['requestId']])->first();
			$BookingData['request_id'] 		= $this->request->data['requestId'];
			$BookingData['vendorId'] 		= $this->request->data['providerId'];
			$BookingData['userId'] 			= $this->request->data['userId'];
			...
			$BookingData['serviceFee'] 		= $this->request->data['service_fee_without_admin_fee'];
			$BookingData['totalAmount'] 	= $this->request->data['total_amount_with_admin_fee'];
			$BookingData['currency'] 		= 'usd';
			$BookingData['transactionId'] 	= $this->request->data['transactionId'];
			$BookingData['tokenId'] 		= $this->request->data['tokenId'];
			$BookingData['payment_mode'] 	= $this->request->data['payment_mode'];
			$BookingData['card_holder_name']= $this->request->data['card_holder_name'];
			...
			$bookings = $this->Requests->find()->where(['Requests.id'=>$this->request->data['requestId']])->contain(['Bookingservices'])->first();
			$vendor = $this->Users->find()->where(['Users.id'=>$bookings->serviceProvider])->first();
			$userS = $this->Users->find()->where(['Users.id'=>$bookings->sender])->first();
			$to = (@$vendor->email) ? @$vendor->email : 'raushan@mailinator.com'; 
			...
			$message .= '<br><strong>Payment Amount</strong>: '.@$this->request->data['service_fee_without_admin_fee'].' <strong>UDS</strong>';
	...
	function addbooking(){
		...
		$this->loadModel('Typeofservices');
		$this->loadModel('Users');
		if($this->request->data('vendorId') !='' && $this->request->data('userId') !='' && $this->request->data('typeOfServices') !=''){
			//echo "<pre>"; print_r($this->request->data); //die;
			$TOTALAMOUNT = 0;$adminFee = 5;
			$Booking = $this->Bookings->newEntity();
			$INVO['vendorId'] 			= $this->request->data('vendorId');
			$INVO['userId'] 			= $this->request->data('userId');
			$INVO['typeofbusinessId']	= $this->request->data('typeofbusinessId');
			$INVO['UTYPE'] 				= $this->request->data('UTYPE');
			$INVO['typeOfServices'] 	= $this->request->data('typeOfServices');
			$INVO['current_time_zone']	 = $this->request->data('current_time_zone');

			$INVO['keyword_1']	 = @$this->request->data('keyword_1');
			$INVO['keyword_2']	 = @$this->request->data('keyword_2');
			$INVO['keyword_3']	 = @$this->request->data('keyword_3');

			//$EXP = json_decode($this->request->data['servicelist']);
			//foreach($EXP as $VV){
			//	$TOTALAMOUNT +=$VV->price;
			//}
			$INVO['adminFee'] 	= $adminFee;
			$INVO['totalAmount'] 		= ($this->request->data['amount'] > 5) ? ($this->request->data['amount']-$adminFee) : 0;
			$EXXP = explode("-", $this->request->data('slotTime'));
			$INVO['slotTime'] 		= date('H:i', strtotime(trim($EXXP[0])))."-".date('H:i', strtotime(trim($EXXP[1])));
			$INVO['comment'] 		= '';
			$INVO['transactionId'] 	= $this->request->data('transactionId');
			$INVO['payment_mode'] 	= $this->request->data('payment_mode');
			$INVO['status'] 		= 1;
			...
			$Booking['added_time'] = date('Y-m-d H:i:s',strtotime($this->request->data['added_time']));
			$Booking['created'] 	= date('Y-m-d H:i:s');
			$this->Bookings->save($Booking);
			...
			$serviceNAME = array();
			$EXP = explode(",", $this->request->data['typeOfServices']);
			...
			$userS = $this->Users->find()->where(['Users.id'=>$this->request->data['userId']])->first();
			$vendorS = $this->Usersinformations->find()->where(['Usersinformations.userId'=>$this->request->data['vendorId'], 'Usersinformations.UTYPE'=>$this->request->data['UTYPE']])->contain(['Users'])->first();
			if(@$vendorS->id !=''){
				...
				$message .= '<br>Your appointment with '.$userS->firstName.' is on '.$this->request->data['bookingDate'].' at '.$this->request->data['slotTime'].' for the service(s) '.implode(", ", $serviceNAME);
				$message .= '<br><br>We look forward to seeing you then.';
				$message .= '<br><br>Thanks,';
				$this->phpemail($to, $subject, $message);
				$deviceToken = @$vendorS->user->deviceToken;
				$body['message'] = 'Your appointment with '.$userS->firstName.' is on '.$this->request->data['bookingDate'].' at '.$this->request->data['slotTime'].' for the service(s) '.implode(", ", $serviceNAME);;
				$body['type'] = 'text';
			...
			if(@$userS->id !=''){
				...
				$message .= '<br><strong>Booking Date</strong>: '.$this->request->data['bookingDate'];
				$message .= '<br><strong>Booking Slot</strong>: '.$this->request->data['slotTime'];
				$message .= '<br><strong>Service Name</strong>: '.implode(", ", $serviceNAME);
				$this->phpemail($to, $subject, $message);
				$deviceToken = @$userS->deviceToken;
				$body['message'] = 'Your appointment has been booked successfully. Business Name:'.@$vendorS->VBusinessName.', Booking Date:'.$this->request->data['bookingDate'].', Booking Slot:'.$this->request->data['slotTime'].", ".implode(", ", $serviceNAME);
				$body['type'] = 'text';
				//$this->sendNotificationOnAndroid($deviceToken,$body);
	...
	function bookingdetail($bookingId=NULL, $user_id=NULL){
		...
		$TypeOfBusinessList = $this->Typeofbusines->find('list', ['keyField' => 'id', 'valueField' => ['name']])->where(['Typeofbusines.status'=>1])->order(['Typeofbusines.name'=>'ASC'])->toArray();
		$userId = ($user_id) ? $user_id : $this->request->data('userId');
		$bookingID = ($bookingId) ? $bookingId : $this->request->data('bookingId');
	...
	function bookingcomplete(){
		...
		if($this->request->data['userId'] !='' && $this->request->data['bookingId'] !='' && $this->request->data['status'] !=''){
			$bookingD = $this->Bookings->find()->where(['Bookings.id'=>$this->request->data['bookingId']])->contain(['Vendors'])->first();
			if($bookingD->id !=''){
				...
				$query->update()
					...
					->execute();
				if($this->request->data['status'] == 2 && $bookingD->UTYPE == 2){
					$u_info = $this->Usersinformations->find()->where(['Usersinformations.UTYPE'=>2,'Usersinformations.userId'=>$this->request->data['userId']])->first();
	...
	function mobileavailability(){
		if($this->request->data['vendorId'] !='' && $this->request->data['openday'] !='' && $this->request->data['mobileprice'] !=''){
			$this->loadModel('Mobileavailability');
			//echo "<pre>";
			//print_r($this->request->data);//die;
			$vendor = $this->Mobileavailability->find()->where(['Mobileavailability.userId'=>$this->request->data['vendorId']])->first();
			if(@$vendor->id == ''){
				$vendor = $this->Mobileavailability->newEntity();
			}
			$DATA['userId'] 		= $this->request->data['vendorId'];
			$DATA['mobileprice'] 	= @$this->request->data['mobileprice'];
			
			$exp = explode(",", $this->request->data['openday']);
			$DATA['MON'] = 0;
			$DATA['TUE'] = 0;
			$DATA['WED'] = 0;
			$DATA['THU'] = 0;
	...
	function mobileavailability_get(){
		$this->loadModel('Mobileavailability');
		$userTYPE = (@$this->request->data['userType']) ? @$this->request->data['userType'] : 2;
		if($this->request->data['vendorId'] !=''){
			$vendor = $this->Mobileavailability->find()->where(['Mobileavailability.userId'=>$this->request->data['vendorId']])->first();
			if(@$vendor !=''){
				$DATA['id'] 						= $vendor->id;
	...
	function setting(){
		if($this->request->data['vendorId'] !='' && $this->request->data['openday'] !='' && $this->request->data['UTYPE'] !='' && $this->request->data['price'] !='' && $this->request->data['time_slot_duration'] !='' ){
			$this->loadModel('Videochatavailability');
			//echo "<pre>";
			//print_r($this->request->data);//die;
			$this->Videochatavailability->deleteAll(['Videochatavailability.userId'=>$this->request->data['vendorId'],'Videochatavailability.UTYPE'=>$this->request->data['UTYPE']]);
			$DATA['UTYPE'] 			= $this->request->data['UTYPE'];
			$DATA['userId'] 		= $this->request->data['vendorId'];
			$DATA['price'] 			= @$this->request->data['price'];
			$DATA['total_price'] 	= ((@$this->request->data['price']*116)/100);
			$DATA['time_slot_duration']= @$this->request->data['time_slot_duration'];
			
			$j_Data = json_decode($this->request->data['openday'], true);
			//print_r($j_Data);
	...
	function getsetting(){
		...
		$userTYPE = (@$this->request->data['userType']) ? @$this->request->data['userType'] : 2;
		if($this->request->data('vendorId') !=''){
			$vendor = $this->Videochatavailability->find()->where(['Videochatavailability.UTYPE'=>$userTYPE,'Videochatavailability.userId'=>$this->request->data('vendorId')])->toArray();
			if(@$vendor){
				$DATA['id'] 						= $vendor[0]->id;
		...
		return 	$response;
			...
			//			$vendorId = ($vendorID) ? $vendorID : $this->request->data('vendorId');
			//			$Date = ($DATE) ? $DATE : $this->request->data('date');
			...
			function vendorslot($vendorID=NULL, $DATE=NULL,$userType=NULL){
				...
				if(@$vendor->id !=''){
					...
					foreach($slot as $KEY=>$Veb){
						...
						$customer_sign = substr(@$this->request->data('current_time_zone'), 0, 1);
						$Vendor_time_Zone = explode(":",substr(@$vendor->current_time_zone,1));
						$customer_time_Zone = explode(":",substr(@$this->request->data('current_time_zone'),1));
						...
						$current_date_time_start = $this->request->data('date').' '.date('H:i',strtotime($exp_slot[0])).':00';
						$current_date_time_end = $this->request->data('date').' '.date('H:i',strtotime($exp_slot[1])).':00';
						$CAL = abs($Final*60);
						if($sign == '+'){
						...
						if(date('Y-m-d',$NOW_start) ==  $this->request->data('date')){
	...
	private function login($slug=NULL){
		if($this->request->data){
			...
			$this->loadModel('Users');
			//$usr = $this->Users->find()->where(['Users.email' => $this->request->data('email')])->first();
			$usr = $this->Users->find()->where(['OR'=>[['Users.email' => $this->request->data('email')],['Users.contactNumber' => $this->request->data('email')]]])->first();
				if(!empty($usr)){
	...
	private function addcontact(){
		...
		if($this->request->data('name') !=''){
				...
				$users = $this->Contacts->patchEntity($users, $this->request->data, ['validate' => false]);
				$users['status'] = 1;
	...
	private function addproduct(){
		...
		if($this->request->data['SKU'] !='' && $this->request->data['userId'] !='' && $this->request->data['categoryId'] !='' && $this->request->data['productName'] !='' && $this->request->data['quantity'] !=''){
				@$PreV = $this->Products->find()->where(['Products.SKU'=>$this->request->data['SKU']])->first();
				if(@$PreV->id ==''){
						$products = $this->Products->newEntity();
						$products = $this->Products->patchEntity($products, $this->request->data, ['validate' => false]);
						$products['status'] = 1;
						...
						if ($this->Products->save($products)) {
								...
								$res = $this->productlist($this->request->data['userId']);
								$response['data'] = $res['data'];
						}
				}else{
					$response['status'] = 'Fails';
	...
	private function editproduct(){
		$this->loadModel('Products');
		if($this->request->data('productId') !=''){
			$this->loadModel('Users');
			$products = $this->Products->find()->where(['Products.id' => $this->request->data('productId')])->first();
			if(!empty($products)){
		...
		private function productlist($userId=NULL){
			...
			if(@$this->request->data['freestuff'] == 1){
			...
			if($USERID !='' && @$this->request->data['ownlist'] == 1){
		...
		private function productdetails($productId=NULL, $userId=NULL){
				...
				@$ProductID = ($productId) ? $productId : $this->request->data('productId');
				@$USERID = ($userId) ? $userId : $this->request->data('userId');
				$Darray = array();
					if($ProductID){
		...
		private function productstatus(){
			$this->loadModel('Products');
			if($this->request->data('productId') !='' && $this->request->data('status') !=''){
				$this->loadModel('Users');
				$products = $this->Products->find()->where(['Products.id' => $this->request->data('productId')])->first();
				if(!empty($products)){
					//pr($products); die;
		...
		function order(){
			...
			$this->loadModel('Products');
			$this->loadModel('Users');
			if($this->request->data['userId'] !='' && $this->request->data['productList'] !=''){
				//$users = $this->Users->find()->Where(['Users.id' => $this->request->data('vendorId')])->first();
				//pr($this->request->data);
				$carts_details = $this->Carts->find()->where(['Carts.userId'=>$this->request->data['userId'], 'Carts.orderId is'=>null])->group('productOwner')->toArray();
				$arr = json_decode($this->request->data['productList']);
				...
				foreach($carts_details as $cart_d){
					$get_D = $this->Carts->find()->where(['Carts.productOwner'=>$cart_d->productOwner,'Carts.userId'=>$this->request->data['userId'], 'Carts.orderId is'=>null])->toArray();
					//pr($get_D); die;
					...
					$DATA['adminFees'] 				= @$this->request->data['adminFees'];
					$DATA['paidAmount'] 			= @$this->request->data['TotalAmount'];
					
					$DATA['created'] 				= date('Y-m-d H:i:s');
					$DATA['status'] 				= 1;
					$DATA['orderStatus']			= 1;
					$DATA['transactionID']			= @$this->request->data['transactionID'];
					$DATA['paymentTrouugh']			= @$this->request->data['paymentTrouugh'];

					$DATA['paymentStatus']			= 'paid';
					
					$DATA['shippingName']			= @$this->request->data['shippingName'];
					$DATA['shippingAddress']		= @$this->request->data['shippingAddress'];
					$DATA['ShippingCity']			= @$this->request->data['ShippingCity'];
					$DATA['ShippingState']			= @$this->request->data['ShippingState'];
					$DATA['shippingZipcode']		= @$this->request->data['shippingZipcode'];
					$DATA['ShippingMobile']			= @$this->request->data['ShippingMobile'];
					$DATA['current_time_zone']		= @$this->request->data['current_time_zone'];
					...
					$Userservices['added_time'] 			= date('Y-m-d H:i:s',strtotime(@$this->request->data['added_time']));
					...
					$query->update()
						...
						->where(['userId' => $this->request->data['userId'], 'productOwner'=>@$cart_d->productOwner, 'orderId is' => null])
						->execute();
				...
				//@$this->Carts->deleteAll(['Carts.userId' => $this->request->data['userId']]);
					
				$buyer = $this->Users->find()->where(['Users.id'=>$this->request->data['userId']])->first();
		...
		function orderlist($userId=NULL, $type=NULL){
			$this->loadModel('Orderes');
			$VendorID = ($userId)? $userId : $this->request->data['userId'];
			@$TYPE = ($type)? $type : @$this->request->data['type'];
			@$pageNo = ($this->request->data['pageNo']) ? $this->request->data['pageNo'] : 1;
			if($VendorID !=''){
				$this->paginate = [
					'limit' => '10',//$limit,
				...
				if(@$this->request->data['track_order'] !=''){
					$condition[] = ['Orderes.order_number'=>$this->request->data['track_order']];
				}
				if($TYPE == 'OWNER'){
	...
	function orderdetails($orderId =NULL, $userId=NULL){
		$this->loadModel('Carts');
		$VendorID = ($userId)? $userId : $this->request->data['userId'];
		$OrderID = ($orderId)? $orderId : $this->request->data['orderId'];
		//@$pageNo = ($this->request->data['pageNo']) ? $this->request->data['pageNo'] : 1;
		if($VendorID !='' && $OrderID !=''){
	...
	function ordertracke($orderId =NULL, $userId=NULL){
		$this->loadModel('Carts');
		$VendorID = ($userId)? $userId : $this->request->data['userId'];
		$OrderID = ($orderId)? $orderId : $this->request->data['order_number'];
		//@$pageNo = ($this->request->data['pageNo']) ? $this->request->data['pageNo'] : 1;
		if($VendorID !='' && $OrderID !=''){
		...
		function orderdelivered(){
			...
			if($this->request->data['orderId'] !='' && $this->request->data['vendorId'] !='' && $this->request->data['status'] !=''){
				$orders = $this->Orderes->find()->where(['Orderes.id'=>$this->request->data['orderId']])->contain(['Carts'])->first();
				//pr($orders); die;
				
				if(@$orders->id !=''){
					$shippingCode = (@$this->request->data['shippingCode']) ? @$this->request->data['shippingCode'] : '0';
					$articles = TableRegistry::get('Orderes');
					$query = $articles->query();
					$query->update()
						->set(['shippingDate'=>date('Y-m-d H:i:s'),'shippingCode'=>$shippingCode, 'orderStatus' => $this->request->data['status']])
						->where(['id' => $orders->id])
						->execute();
					
					if($this->request->data['status'] == 3){
						$user_d = $this->Users->find()->where(['Users.id'=>$this->request->data['vendorId']])->first();
		...
		function deliveryconfirm(){
			$this->loadModel('Orderes');$this->loadModel('Carts');$this->loadModel('Users');
			if($this->request->data['orderId'] !='' && $this->request->data['userId'] !='' && $this->request->data['status'] !=''){
				$orders = $this->Orderes->find()->where(['Orderes.id'=>$this->request->data['orderId']])->contain(['Carts'])->first();
				//pr($orders); die;
				...
				if(@$orders->id !=''){
					...
					$query = $articles->query();
					$query->update()
						->set(['deliveryConfirm'=>$this->request->data['status']])
						->where(['id' => $orders->id])
						->execute();
		...
		function addtocart(){
			$this->loadModel('Carts');
			if($this->request->data['userId'] !='' && $this->request->data['productId'] !='' && $this->request->data['quantity'] !=''){
			$carts = $this->Carts->newEntity();
				$carts = $this->Carts->patchEntity($carts, $this->request->data, ['validate' => false]);
				$carts['status'] = 1;
		...
		function editcart(){
			$this->loadModel('Carts');
			if($this->request->data['userId'] !='' && $this->request->data['cartId'] !=''){
				$carts = $this->Carts->find()->where(['Carts.userId'=>$this->request->data['userId'], 'Carts.id'=>$this->request->data['cartId']])->first();
				if(@$carts->id !='') {
					$carts = $this->Carts->patchEntity($carts, $this->request->data, ['validate' => false]);
					...
					$response['status'] = 'success';
					$response['msg'] = 'Data save successfully.';
					$DD = $this->cartlist($this->request->data['userId']);
					$response['data'] = $DD['data'];
				}else{
					$response['status'] = 'Fails';
				}
		...
		private function deletecart(){
			...
			if($this->request->data('cartId') !='' && $this->request->data('userId') !=''){
				$listing = $this->Carts->find()->where(['Carts.id'=>$this->request->data('cartId'), 'Carts.userId'=>$this->request->data('userId')])->first();
				if($listing) {
			...
			private function addstaff(){
					$this->loadModel('Freestaffs');
					$this->loadModel('Products');
					if($this->request->data['postTitle'] !='' && $this->request->data['userId'] !='' && $this->request->data['categoryId'] !='' && $this->request->data['description'] !='' && $this->request->data['image_1'] !=''){
							//@$PreV = $this->Products->find()->where(['Products.SKU'=>$this->request->data['SKU']])->first();
							
									$products = $this->Freestaffs->newEntity();
									$products = $this->Freestaffs->patchEntity($products, $this->request->data, ['validate' => false]);
									$products['status'] = 1;
									$products['created'] = date('Y-m-d H:i');
									$fileName1 = '';
									$fileName2 = '';
									...
									if ($this->Freestaffs->save($products)) {
										if(@$_FILES['video']['name'] !='' && $videoName !=''){
											...
											if(@$_FILES['image_5']['name'] !='' && $fileName5 !=''){
												...
												$res = $this->stufflist($this->request->data['userId']);
												$response['data'] = $res['data'];
									}
			...
			private function editstaff(){
					$this->loadModel('Freestaffs');
					$this->loadModel('Products');
					if($this->request->data['staffId'] !='' && $this->request->data['userId'] !=''){
							$products = $this->Freestaffs->find()->where(['Freestaffs.id'=>$this->request->data['staffId']])->first();
							if(@$products){
									$products = $this->Freestaffs->patchEntity($products, $this->request->data, ['validate' => false]);
									$products['status'] = 1;
									$products['created'] = date('Y-m-d H:i');
			...
			private function stuffdetail($staffId=NULL){
				...
				$this->loadModel('Freestafflikes');
				$this->loadModel('User');
				@$USERID = ($staffId) ? $staffId : $this->request->data('staffId');
				@$pageNo = ($this->request->data['pageNo']) ? $this->request->data['pageNo'] : 1;
				...
				//echo "<pre>"; print_r($userservices); die;
					...
					if(@$this->request->data['userId'] !=''){
						@$likes = $this->Freestafflikes->find()->where(['Freestafflikes.freestaffId'=>$val->id,'Freestafflikes.userId'=>$this->request->data['userId']])->first();
					}
			...
			function stuffview($staffId=NULL){
					...
					if($USERID !=''){
							...
							$query->update()
								->set(['totalView' => $TOTAL])
								->where(['id' => $this->request->data['staffId']])
								->execute();
					}
				$response['status'] = "success";
					return  $response;
	...
	private function deletestuff(){
		...
		if($this->request->data('freestaffId') !='' && $this->request->data('userId') !=''){
			$listing = $this->Freestaffs->find()->where(['Freestaffs.id'=>$this->request->data('freestaffId'), 'Freestaffs.userId'=>$this->request->data('userId')])->first();
			if($listing) {
	...
	private function deletestuffimage(){
		...
		if($this->request->data('freestaffId') !='' && $this->request->data('userId') !='' && $this->request->data['image_key'] !=''){
			$listing = $this->Freestaffs->find()->where(['Freestaffs.id'=>$this->request->data('freestaffId'), 'Freestaffs.userId'=>$this->request->data('userId')])->first();
			if($listing) {
				$dd = $this->request->data['image_key'];
				$IMG_U = $listing->$dd;
				$PATH = WWW_ROOT.'/img/uploads/freestaff/';
					@unlink($PATH.$IMG_U);
					$articles = TableRegistry::get('Freestaffs');
					...
					$query->update()
						->set([$dd => ''])
						->where(['id' => $this->request->data['freestaffId']])
						->execute();
					$response['status'] = 'success';
		...
		return  $response;
			...
			function stufflikes(){
				if($this->request->data['status'] !='' && $this->request->data['userId'] !=''){
					$this->loadModel('Freestafflikes');
					$articleId = (@$this->request->data['freestaffId']) ? @$this->request->data['freestaffId'] : '';
					if($articleId !=''){
								$likes = $this->Freestafflikes->find()->where(['Freestafflikes.freestaffId'=>$articleId,'Freestafflikes.userId'=>$this->request->data['userId']])->first();
							if($this->request->data['status'] == 0){
								if(@$likes->id !=''){
									...
									if(@$likes->id ==''){
										...
										@$this->request->data['status'] = ($this->request->data['status']) ? $this->request->data['status'] : 0;
										$likes = $this->Freestafflikes->patchEntity($likes, $this->request->data, ['validate' => false]);
		...
		function addcomment(){
			$this->loadModel('Freestaffcomments');
			$this->loadModel('Freestaffs');
				if($this->request->data['userId'] !='' && $this->request->data['freestaffId'] !='' && $this->request->data['comment'] !=''){
					if(@$this->request->data['commentId'] !=''){
						$comments = $this->Freestaffcomments->find()->where(['Freestaffcomments.id'=>$this->request->data['commentId'],'Freestaffcomments.userId'=>$this->request->data['userId']])->first();
						if(@$comments->id ==''){
					...
					$comments = $this->Freestaffcomments->patchEntity($comments, $this->request->data, ['validate' => false]);
					$comments['status'] 	= 1;
					$comments['created'] 	= date('Y-m-d H:i:s');
					$comments['added_time'] = date('Y-m-d H:i:s',strtotime($this->request->data['added_time']));
					if($this->Freestaffcomments->save($comments)){
						$listingD = $this->Freestaffs->find()->where(['Freestaffs.id' => $this->request->data['freestaffId']])->first();
				...
				function commentdelete(){
					$this->loadModel('Freestaffcomments');
					$this->loadModel('Freestaffs');
					if($this->request->data['userId'] !='' && $this->request->data['freestaffId'] !='' && $this->request->data['commentId'] !=''){
						$comments = $this->Freestaffcomments->find()->where(['Freestaffcomments.id'=>$this->request->data['commentId'],'Freestaffcomments.freestaffId'=>$this->request->data['freestaffId'],'Freestaffcomments.userId'=>$this->request->data['userId']])->first();
							if(@$comments->id !=''){
				...
				function commentlist($freestaffId=NULL,$PAGENO=NULL){
								$this->loadModel('Freestaffcomments');
								@$pageNo = ($PAGENO) ? $PAGENO : ($this->request->data['pageNo'] ? $this->request->data['pageNo'] : '1');
								@$freestaffID = ($freestaffId) ? $freestaffId : $this->request->data['freestaffId'];
								if($pageNo !=''){
										$this->paginate = [
											'limit' => '10',//$limit,
											'order' => [
										...
										$keyword = $this->request->query('keyword');
										$condition[] = ['Freestaffcomments.freestaffId'=>$this->request->data['freestaffId'],'Freestaffcomments.status'=>1];
				...
				function searchnew(){
						$this->loadModel('Users');
						$this->loadModel('Specializations');
						if(isset($this->request->data['typeOfBusiness'])){
							$typeOfBusiness = $this->request->data['typeOfBusiness'];
							$keyword = $this->request->data['keyword'];
							//$subCategoryId = $this->request->data['categoryId'];
							$this->paginate = [
								'limit' => '50',//$limit,
								//'order' => ['Users.id' => 'desc'],
								'order' => ['rand()'],
		...
		function searchdetail($userId = NULL){
			...
			$latitude = '';
			$longitude = '';
			if(trim(@$this->request->data['lat']) !=''){
				$latitude = $this->request->data['lat'];
			}
			if(trim(@$this->request->data['longs']) !=''){
				$longitude = $this->request->data['longs'];
			}
			//$userD = $this->Users->find()->where(['Users.id' => $userID])->contain(['Countries','States'])->first();
	...
	function submitreview(){
		if($this->request->data['reviewTo'] !='' && $this->request->data['reviewFrom'] !='' && $this->request->data['star'] != '' && $this->request->data['message'] !=''){
				
			$this->loadModel('Reviews');
			$this->loadModel('Users');
			// $reviews = $this->Reviews->find()->where(['Reviews.bookingId'=>$this->request->data['bookingId'],'Reviews.reviewTo'=>$this->request->data['reviewTo'],'Reviews.reviewFrom'=>$this->request->data['reviewFrom']])->first();
			// if(@$reviews->id !=''){
			// 	$response['status'] = 'Fail'; 
			...
			$users = $this->Users->find()->where(['Users.id'=>$this->request->data['reviewTo']])->first();
	...
	private function reviewlist($reviewfrom=NULL, $reviewto = NULL){
		...
		@$USERID = ($reviewfrom) ? $reviewfrom : $this->request->data['reviewfrom'];
		@$Reviewto = ($reviewto) ? $reviewto : $this->request->data['reviewto'];
		@$pageNo = ($this->request->data['pageNo']) ? $this->request->data['pageNo'] : 1;
		if($Reviewto !=''){
			$this->paginate = [
				'limit' => '10',//$limit,
	...
	function reviewproduct(){
		if($this->request->data['productId'] !='' && $this->request->data['reviewFrom'] !='' && $this->request->data['star'] != '' && $this->request->data['message'] !=''){
			...
			$reviews['status'] 	= '1';
			$reviews['added_from'] = 'WED';
			//$reviews['added_time'] = date('Y-m-d H:i:s',strtotime(@$this->request->data['added_time']));
			$this->Reviews->save($reviews);
			
			$query = $this->Reviews->find()->where(['Reviews.productId'=>$this->request->data['productId']]);
			$avg = $query->select(['averagerating' => $query->func()->avg('star')])->toArray();
			//echo "<pre>"; print_r($avg);
			//echo $avg[0]->averagerating;
			//die;
			...
			$ABG['AVGRating'] = round($avg[0]->averagerating,2);
			$users = $this->Products->find()->where(['Products.id'=>$this->request->data['productId']])->first();
	...
	function typeofbusiness(){
			$this->loadModel('Typeofbusines');
			$TYPE = @$this->request->data('type');//Other  // Veterinarian
	...
	function typeofservice(){
			$this->loadModel('Typeofservices');
			$parentCountryList = $this->Typeofservices->find()->where(['Typeofservices.status'=>1,'Typeofservices.UTYPE'=>$this->request->data['UTYPE']])->order(['Typeofservices.name'=>'ASC'])->toArray();
			$PC = array();
			foreach($parentCountryList as $key=>$VV){
				$PC['id'] = $VV->id;
				$PC['name'] = $VV->name;
	...
	function statelist($counttyId = NULL){
		...
		$counttyID = ($counttyId) ? $counttyId : @$this->request->data('counttyId');
		if(@$this->request->data['state_name'] !=''){
			$parentCountryList = $this->States->find()->where(['States.country_id'=>$counttyID,'States.name Like'=>$this->request->data['state_name'].'%'])->order(['States.name'=>'ASC'])->toArray();
		}else{
	...
	function citylist($counttyId = NULL){
		...
		$counttyID = ($counttyId) ? $counttyId : @$this->request->data('state_id');
	...
	//	$this->loadModel('Categories');
	//	$conditons = array('Categories.status' => '1' , 'Categories.parentId' => $this->request->data['parentId'] );
	...
	function sendnotification($id=NULL){
		if($this->request->data['Token'] !='' && $this->request->data['message'] !='' && $this->request->data['device']){
			$body['message'] = $this->request->data['message'];
			$body = $this->request->data;
			if(strtolower($this->request->data['device']) == 'ios'){
				$dd = $this->sednIosPushNotification($this->request->data['Token'],$body);
			}else{
				$dd = $this->sendNotificationOnAndroid($this->request->data['Token'],$body);
			}
			$response['status'] = "success";
	...
	function updatepayment(){
		if($this->request->data['UTYPE'] !='' && $this->request->data['userInfoId'] !='' && $this->request->data['userId'] !='' && $this->request->data['SubscriptionFrom'] !=''){
			...
			//$Previous_day = date('Y-m-d',strtotime($this->request->data['SubscriptionFrom']." +30 days"));
			
			$articles = TableRegistry::get('Usersinformations');
			$query = $articles->query();
			$query->update()
				->set(['SubscriptionFrom'=>$this->request->data['SubscriptionFrom'],'purcheseDate'=>date('Y-m-d'),'expiryDate'=>$Previous_day])
				->where(['UTYPE'=>$this->request->data['UTYPE'],'id' => $this->request->data['userInfoId']])
				->execute();
			
				$userINFO = $this->Usersinformations->find()->where(['Usersinformations.id'=>$this->request->data['userInfoId']])->contain(['Users'])->first();
		...
		function cancelsubscription(){
			if($this->request->data['userId'] !='' && $this->request->data['stripeSubscriptionId'] !=''){
		...
		function getwallet(){
			if($this->request->data['userId'] !=''){
				...
				$userinfoo = $this->Usersinformations->find()->where(['Usersinformations.userId'=>$this->request->data['userId'],'Usersinformations.UTYPE'=>2])->contain(['Users'])->first();
				$cahsout = $this->Cashouts->find()->select(['sum' => 'SUM(Cashouts.approved_amount)'])->where(['Cashouts.userId'=>$this->request->data['userId'], 'Cashouts.status'=>2,'Cashouts.status'=>2])->toArray();
				...
				$userinfoo2 = $this->Users->find()->where(['Users.id'=>$this->request->data['userId']])->first();
		...
		function cashoutrequest(){
			if($this->request->data['userId'] !='' && $this->request->data['requested_amount'] !='' && $this->request->data['type'] !=''){
				$this->loadModel('Cashouts');$this->loadModel('Users');
				$Cashouts = $this->Cashouts->newEntity();
				$Cashouts = $this->Cashouts->patchEntity($Cashouts, $this->request->data, ['validate' => false]);
				$Cashouts['created'] = date('Y-m-d H:i:s');
				//$Cashouts['added_time'] = date('Y-m-d H:i:s',strtotime(@$this->request->data['added_time']));
				...
				$message="Dear Admin";
				$message .= '<br>One service provider has requested for cashout amount of <strong>$'.$this->request->data['requested_amount'].'</strong>';
				$message .= '<br>';
		...
		function callhistoryadd(){
			if($this->request->data['callerId'] !='' && $this->request->data['receiverId'] !='' && $this->request->data['duration'] !='' && $this->request->data['callerType'] != '' ){
					
				$this->loadModel('Callhistories');
				$callhistory = $this->Callhistories->newEntity();
				$callhistory = $this->Callhistories->patchEntity($callhistory, $this->request->data, ['validate' => false]);
		...
		function callhistorydelete(){
			if($this->request->data['callerId'] !='' && $this->request->data['receiverId'] !='' && $this->request->data['callhistoryId'] !='' ){
				$this->loadModel('Callhistories');
				$callhistory = $this->Callhistories->find()->where(['Callhistories.id'=>$this->request->data['callhistoryId'],'Callhistories.receiverId'=>$this->request->data['receiverId'],'Callhistories.callerId'=>$this->request->data['callerId']])->first();
				if(@$callhistory->id !=''){
	...
	public function addmultiimage(){
		$this->loadModel('Images');
		if ($this->request->data['userId'] != '' && $this->request->data['UTYPE'] != '' && $this->request->data['userInfoId'] != '' && $this->request->data['imageType'] != '') {
			if (@$_FILES['multiImage']['error']) {
				foreach (@$_FILES['multiImage']['error'] as $key => $val) {
					if ($val == '0') {
						$DATA['userId'] 		= $this->request->data['userId'];
						$DATA['UTYPE'] 			= $this->request->data['UTYPE'];
						$DATA['userInfoId'] 	= $this->request->data['userInfoId'];
						$DATA['imageType'] 		= $this->request->data['imageType'];
						$DATA['image'] = '';
						$fileName1 = '';
	...
	function addmultiimageIOS(){
		...
		if ($this->request->data['userId'] != '' && $this->request->data['UTYPE'] != '' && $this->request->data['userInfoId'] != '' && $this->request->data['imageType'] != '') {
			if(@$_FILES['multiImage']['error']){
				@mkdir(WWW_ROOT.'img/uploads/table/'.$this->request->data['userId']);
				foreach(@$_FILES['multiImage']['error'] as $key=>$val){
					if($val == '0'){
						$DATA['userId'] 			= $this->request->data['userId'];
						$DATA['clubTableId'] 		= (@$this->request->data['clubTableId']) ? @$this->request->data['clubTableId'] : '';
						$DATA['image'] = '';
						$fileName1 = '';
						if(@$_FILES['multiImage']['name'][$key] !=''){
							$fileName1=time().stripslashes($_FILES['multiImage']['name'][$key]);
						...
						if(@$_FILES['multiImage']['name'][$key] !='' && $fileName1 !=''){
							$target = WWW_ROOT.'/img/uploads/table/'.$this->request->data['userId'].'/'.$fileName1;
							$source=$_FILES['multiImage']['tmp_name'][$key];
							move_uploaded_file($source,$target);
						}
					}
	...
	function multiimagedelete(){
		if ($this->request->data['userId'] != '' && $this->request->data['multiimageId'] != '' && $this->request->data['imageType'] != '') {
			$this->loadModel('Images');
			$galleries = $this->Images->find()->where(['Images.userId'=>$this->request->data['userId'],'Images.id'=>$this->request->data['multiimageId'],'Images.imageType'=>$this->request->data['imageType']])->first();
			if($galleries->id !=''){
	...
	public function getTaxRate($zip=NULL) 
    {
		...
		$zips = (@$zip) ? @$zip : @$this->request->data['zip'];
        // API SANDBOX URL
	...
	private function e_artical_list($e_category_id=NULL){
		...
		@$ECID = ($e_category_id) ? $e_category_id : $this->request->data['e_category_id'];
		@$pageNo = ($this->request->data['pageNo']) ? $this->request->data['pageNo'] : 1;
		//echo $ECID; die;
		if($ECID !=''){
			$this->paginate = [
